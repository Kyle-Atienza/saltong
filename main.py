...
import grid
import keyboard
from modals import finished_modal, message_modal, instructions_modal
import start_screen
...
while True:
        ...
        if start:
            ...
            if event.type == pygame.KEYDOWN and not message["show"]:
                ...
                if event.key == pygame.K_RETURN:
                    ...
                    if grid.current_row + 1 == grid.rows_count and not message["show"] and not grid.get_row_word() == word:
                        finish["show"] = True
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                if event.button == 1:
                    if start_screen.button_start_rect.collidepoint(event.pos):
                        start = True
    ...
    if instructions:
        screen.blit(instructions_modal.surface, instructions_modal.rect)
